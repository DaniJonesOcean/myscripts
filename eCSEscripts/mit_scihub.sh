...
module add mvapich/pgi
module add pgi
module add netcdf/pgi
module add sge

setenv M_ROOT  /data/scihub-users/`whoami`/mitgcm

setenv PATH /data/scihub-data/oceans-input/Programs/nco-4.0.8/bin:${PATH}

# SG: Commented out as mit_allhosts.sh will no longer be used. With the
# start of ARCHER, most scripts will be in bash or python.
#
...
if ( `whoami` == jamrda26 ) then
  setenv ROOTDIR $M_ROOT/MITgcm
  setenv HOMEROOT /data/oceans_output/shelf/jamrda26/mitgcm
endif
